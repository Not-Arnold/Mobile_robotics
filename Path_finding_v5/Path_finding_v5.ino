...
bool getFirstGoalFromServer() {
  String resp = notifyArrival(currentPosition);
  ...
  Serial.print("Server goal received: ");
  Serial.println(goalNode);
  return true;
}

bool computeFirstRoute() {
  if (!haveGoal) return false;

  startNode = currentPosition;

  bool ok = computeRouteTo(startNode, goalNode);  // fills route[] and routeLen
  haveRoute = ok;

  if (ok) {
    Serial.print("Route stored. Len=");
...
void printRouteArray() {
  Serial.print("[ROUTE] len=");
...
void loop() {
  ...
  // 1) Send currentPosition and get server response
  String resp = notifyArrival(currentPosition);
  ...
  if (resp == "Finished") {
    Serial.println("[DONE] Server says Finished.");
    finished = true;
    return;
  }

  // 2) Parse goal node
  int nextGoal = resp.toInt();
  Serial.print("[GOAL] nextGoal=");
  Serial.println(nextGoal);

  // 3) Compute path from currentPosition -> nextGoal
  bool ok = computeRouteTo(currentPosition, nextGoal);

  if (!ok) {
    Serial.println("[ERROR] No route found (unreachable).");
    delay(1000);
    return;
  }

  // 4) Print the stored route
  printRouteArray();

  // 5) IMPORTANT: for this TEST sketch only:
  //    "pretend" we travelled and arrived at the goal so next iteration advances
  currentPosition = nextGoal;

  delay(800); // don’t spam server too hard
}
