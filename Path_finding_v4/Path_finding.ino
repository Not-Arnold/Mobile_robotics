...
int prevNode = -1;
int currNode = 0;          // start node
bool haveRoute = false;

enum Turn { STRAIGHT, LEFT, RIGHT, UTURN };
int destIdx = 1; // destinations[0] already 0, next target is destinations[1]
...
Turn getTurn(int prev, int curr, int next) {
  // --- Node 6 mapping example ---
  ...
  // TODO: add other junction nodes here (curr == 3, curr == 4, etc.)
  return STRAIGHT;}
...
int calculateError() {
  ...
  // Node detection
  if (s0 && s1 && s2 && s3 && s4) {
    static unsigned long int lastHit = 0;
